...
const BOOL = ['require_auth_time', 'backchannel_logout_session_required']
...
const HTTPS_URI = ['initiate_login_uri', 'sector_identifier_uri', 'request_uris']
...
module.exports = function getSchema(provider) {
  const ENUM = {
    ...
    introspection_endpoint_auth_method: () =>
      instance(provider).configuration('introspectionEndpointAuthMethods'),
    introspection_endpoint_auth_signing_alg: () =>
      instance(provider).configuration('introspectionEndpointAuthSigningAlgValues'),
    revocation_endpoint_auth_method: () =>
      instance(provider).configuration('revocationEndpointAuthMethods'),
    revocation_endpoint_auth_signing_alg: () =>
      instance(provider).configuration('revocationEndpointAuthSigningAlgValues'),
    userinfo_signed_response_alg: () =>
      instance(provider).configuration('userinfoSigningAlgValues'),
    ...
    id_token_encrypted_response_alg: () =>
      instance(provider).configuration('idTokenEncryptionAlgValues'),
    id_token_encrypted_response_enc: () =>
      instance(provider).configuration('idTokenEncryptionEncValues'),
    userinfo_encrypted_response_alg: () =>
      instance(provider).configuration('userinfoEncryptionAlgValues'),
    userinfo_encrypted_response_enc: () =>
      instance(provider).configuration('userinfoEncryptionEncValues'),
    request_object_encryption_alg: () =>
      instance(provider).configuration('requestObjectEncryptionAlgValues'),
    request_object_encryption_enc: () =>
      instance(provider).configuration('requestObjectEncryptionEncValues')
  }

  const DEFAULT = {
    application_type: 'web',
    backchannel_logout_session_required: instance(provider).configuration(
    ...
    post_logout_redirect_uris: instance(provider).configuration('features.sessionManagement')
      ? []
      : undefined,
    request_uris: instance(provider).configuration(
      'features.requestUri.requireRequestUriRegistration'
    )
      ? []
      : undefined,
  ...
  class Schema {
    constructor(metadata) {
      ...
      const rts = _.chain(this.response_types).map(rt => rt.split(' ')).flatten().uniq().value()
      ...
      if (this.subject_type === 'pairwise' && !this.sector_identifier_uri) {
        const hosts = _.chain(this.redirect_uris).map(uri => new URL(uri).host).uniq().value()
        ...
        switch (this.application_type) { // eslint-disable-line default-case
          case 'web': {
            ...
            if (this.grant_types.includes('implicit') && protocol === 'http:') {
              ...
              console.log(
                'warning: redirect_uris for web clients using implicit flow SHOULD only register URLs using the https scheme'
              )
            }
            ...
            if (this.grant_types.includes('implicit') && hostname === 'localhost') {
              ...
              console.log(
                'warning: redirect_uris for web clients using implicit flow SHOULD not be using localhost'
              )
            }
            break
          }
            ...
            if (instance(provider).configuration('features.oauthNativeApps')) {
              switch (parsed.protocol) {
                ...
                case 'https:': // Claimed HTTPS URI Redirection
                  if (LOOPBACKS.includes(parsed.hostname)) {
                    invalidate(
                      `redirect_uris for native clients using claimed HTTPS URIs must not be using ${parsed.hostname} as hostname`
                    )
                  }
                  break
                default:
