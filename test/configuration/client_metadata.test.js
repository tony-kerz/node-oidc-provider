...
describe('Client validations', function() {
  ...
  context('grant_types', function() {
    ...
    rejects(this.title, ['implicit'], null, {
      // misses authorization_code
      response_types: ['id_token', 'code']
    })
    rejects(this.title, ['authorization_code'], null, {
      // misses implicit
  ...
  context('token_endpoint_auth_signing_alg', function() {
    ...
    rejects(
      this.title,
      'RS384',
      /^token_endpoint_auth_signing_alg must be one of/,
      {
        token_endpoint_auth_method: 'client_secret_jwt'
      },
      {
    ...
    it('is required when id_token_encrypted_response_enc is also provided', function() {
      return addClient({
        ...
        expect(err.error_description).to.equal(
          'id_token_encrypted_response_alg is mandatory property'
        )
      })
    })
    ;[
  ...
  context('userinfo_encrypted_response_alg', function() {
    ...
    it('is required when userinfo_encrypted_response_enc is also provided', function() {
      return addClient({
        ...
        expect(err.message).to.equal('invalid_client_metadata')
        expect(err.error_description).to.equal(
          'userinfo_encrypted_response_alg is mandatory property'
        )
      })
    })
    ;[
  ...
  context('request_object_encryption_alg', function() {
    ...
    it('is required when request_object_encryption_enc is also provided', function() {
      return addClient({
        ...
        expect(err.message).to.equal('invalid_client_metadata')
        expect(err.error_description).to.equal(
          'request_object_encryption_alg is mandatory property'
        )
      })
    })
    ;[
